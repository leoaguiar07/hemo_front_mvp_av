...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function carregarDados(data) {
        ...
        data.forEach(item => {
            ...
            newRow.insertCell(2).innerHTML = item.hemocentro;
            newRow.insertCell(3).innerHTML = formatarDataHora(item.data_hora);
            newRow.insertCell(4).innerHTML = item.tipo_sanguineo;
            newRow.insertCell(5).innerHTML = item.fator_rh;
            newRow.insertCell(6).innerHTML = item.unidade;
            if (item.status == "disponivel") {
                item.status = "Disponível";
            }
            newRow.insertCell(7).innerHTML = item.status;
        // newRow.insertCell(7).innerHTML = item.peso_ap

             // Cria um elemento de célula para os botões
             const buttonCell = newRow.insertCell(8);
